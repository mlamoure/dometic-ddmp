...
def build_query(service: str = SERVICE_TYPE, *, unicast_response: bool = False) -> bytes:
...
def query_mdns(
    service: str = SERVICE_TYPE, *, timeout: float = 2.0, interface_ip: str | None = None
) -> list[MdnsService]:
    ...
    try:
        ...
        try:
            ...
            sock.bind(("", 0))
            unicast = True
        iface = socket.inet_aton(interface_ip) if interface_ip else socket.inet_aton("0.0.0.0")
        with contextlib.suppress(OSError):
            sock.setsockopt(
                socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(MDNS_GROUP) + iface
            )
        if interface_ip:
            with contextlib.suppress(OSError):
...
def discover(
    *,
    ...
    def remember(d: Discovered) -> None:
        key = d.short_id or f"name:{d.name.lower()}"  # MAC tail is common to both sources
        prev = found.get(key)
        if prev is None:
            found[key] = d
            return
    ...
    if mdns:
        for svc in query_mdns(timeout=timeout):
            ...
            if ip:
                for r in probe_ddmd(ip, svc.port, timeout=probe_timeout, first_only=True):
