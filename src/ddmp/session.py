...
class Session:
    ...
    def set_frame(
        ...
        if tp.per_compartment:
            ...
            if not current:
                raise StateUnknown(f"{tp.name} has not been published yet; subscribe first")
            idx = 0 if compartment is None else compartment
            if idx < 0 or idx >= len(current):
                raise ValueError(f"compartment {idx} out of range for {tp.name} ({len(current)})")
            new = list(current)
            new[idx] = _coerce(tp, value)
            if tp.name == "csettemp":
                self._check_range(idx, new[idx])
            payload = tp.codec.encode(new)
            return Frame(Action.SET, tp.address, payload), WriteExpectation(tp, new, tolerance)
        ...
        return Frame(Action.SET, tp.address, payload), WriteExpectation(tp, expected, tolerance)
    ...
    def _check_range(self, idx: int, setpoint_c: float) -> None:
        ...
        if idx < len(ranges):
            ...
            if not lo <= setpoint_c <= hi:
                raise ValueError(f"set-point {setpoint_c} °C outside cooler range {lo}..{hi} °C")
