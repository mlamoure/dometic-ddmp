...
def cmd_discover(args: argparse.Namespace) -> int:
    if args.mdns_only:
        for svc in query_mdns(timeout=args.timeout):
            print(f"{svc.instance!r} -> {svc.hostname}:{svc.port} {svc.addresses} {svc.txt}")
        return 0
    if args.host:
...
def _connect(args: argparse.Namespace, on_event) -> SyncClient:
    ...
    client = SyncClient(
        args.host, args.port, on_event=on_event, on_close=on_close, connect_timeout=args.timeout
    )
    client.connect()
...
def _write(args: argparse.Namespace, name: str, value: Any, compartment: int | None) -> int:
    events: list[Event] = []
    lock = threading.Lock()
    ...
    try:
        ...
        frame, expectation = client.session.set_frame(name, value, compartment=compartment)
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if tp.name not in WRITABLE:
        print(
            f"{tp.name} is not writable (allowed: {', '.join(sorted(WRITABLE))})", file=sys.stderr
        )
        return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("state", help="subscribe, wait, print the decoded state (read-only)")
    conn_args(p)
    ...
    p = sub.add_parser("set-temp", help="set the set-point in °C (dry run without --yes)")
    write_args(p)
