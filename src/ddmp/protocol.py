...
class MilliPairArrayCodec:
    ...
    def decode(self, payload: bytes) -> list[tuple[float, float]]:
        ...
        return [(values[i] / 1000, values[i + 1] / 1000) for i in range(0, len(values) - 1, 2)]
...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic("cpow", _mccc(0x03), _BOOLS, "Compartment power", writable=True, per_compartment=True),
    Topic("ctemp", _mccc(0x04), _MILLIS, "Measured temperature", per_compartment=True, unit="°C"),
    Topic(
        "csettemp",
        _mccc(0x05),
        _MILLIS,
    ...
    Topic(
        "ctemprng", _mccc(0x08), _PAIRS, "Allowed set-point range", per_compartment=True, unit="°C"
    ),
    Topic("crecdrng", _mccc(0x09), _PAIRS, "Recommended range", per_compartment=True, unit="°C"),
    Topic("ctempofs", _mccc(0x0A), _MILLIS, "Temperature offset", per_compartment=True, unit="°C"),
...
NEVER_WRITE: tuple[tuple[Address, str], ...] = (
    (Address(0x14, 0, 0, 0), "gw.cupd command word: 11223344 = Restart, 19181716 = Factory reset"),
    (Address(0x03, 0, 0, 0), "gw.ota firmware update trigger"),
    (Address(0x06, 0, 0, 0), "gw.awssc AWS certificate slot (06-09 are AWS, 0A-0D are OTA certs)"),
